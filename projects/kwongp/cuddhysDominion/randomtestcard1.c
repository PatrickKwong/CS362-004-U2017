 ...
 *
 * Random test generator for Smithy using teammate(cuddhys)'s dominion code
 * - draws 3 cards
 *
...
int main() {
	...
	for(z = 1; z <= TEST_RUNS; z++) // test for loop
	{
		...
		// smithy_func(G.handCount[0], &G); // my previous code
		// modifications to be able to use teammates's smithy function
		int currentPlayer = 0;
		smithyFunc(currentPlayer, &G, G.handCount[0]);
